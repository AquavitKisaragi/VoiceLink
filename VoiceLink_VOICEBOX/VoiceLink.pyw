...
def talk(outputType,filename):

    try:
        if (outputType == 0 or outputType == 2):
            winsound.PlaySound(filename,  winsound.SND_MEMORY)
    except:
        print ("なんかエラーだって")
        sys.exit(-4)

def audio_query(outputType,text, filename, speaker):
...
def main():
    if len(sys.argv) > 1:
        try:
            os.chdir('VoiceData/')
            outputType = int(1) # 0:voice onry,1:file onry,2,Voice and File
            ...
            audio_query(outputType,text,filename,sparg)
        except:
            print ("なんかエラーだって")
            sys.exit(-3)
